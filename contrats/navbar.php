
<nav id="sidebar" class="active">
            <div class="sidebar-header">
                <h4 class="text-white fw-bold p-3">CRM-GGA</h4>
            </div>
            <ul class="list-unstyled components">
                <li data-section="dashboard">
                    <a href="../dashboard/"><i class="bi bi-house-door"></i><span>Dashboard</span></a>
                </li>
                <li data-section="clients">
                    <a href="../clients"><i class="bi bi-people"></i><span>Clients</span></a>
                </li>
                <li data-section="partenaires">
                <a href="../partenaires/"><i class="bi bi-shield-check"></i><span>partenaires</span></a>
                </li>
                <li class="active" data-section="contrats">
                    <a href="./"><i class="bi bi-folder"></i><span>Contrats</span></a>
                </li>
                <li data-section="Tâches">
                    <a href="../taches/"><i class="bi bi-list-task"></i><span>Tâches</span></a>
                </li>
                <li data-section="chatbox">
                    <a href="#"><i class="bi bi-chat-dots"></i><span>ChatBox</span></a>
                </li>
                <li data-section="stats">
                    <a href="#"><i class="bi bi-graph-up"></i><span>Statistiques</span></a>
                </li>
                <li data-section="messages">
                    <a href="#"><i class="bi bi-envelope"></i><span>Messages</span></a>
                </li>
                <li data-section="share">
                    <a href="#"><i class="bi bi-share"></i><span>Partage</span></a>
                </li>
                <li data-section="Utilisateurs">
                    <a href="../users/"><i class="bi bi-person-circle"></i><span>Utilisateurs</span></a>
                </li>
            </ul>
            <div class="sidebar-footer">
                <a href="#"><i class="bi bi-gear"></i><span>Paramètres</span></a>
            </div>
        </nav>
